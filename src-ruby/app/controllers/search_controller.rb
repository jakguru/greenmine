...
class SearchController < ApplicationController
  ...
  def index
    ...
    if (request.xhr?)
      searchableProjects = [[l(:label_project_all), 'all']]
      searchableProjects << [l(:label_my_projects), 'my_projects'] unless User.current.memberships.empty?
      searchableProjects << [l(:label_my_bookmarks), 'bookmarks'] unless User.current.bookmarked_project_ids.empty?
      searchableProjects << [l(:label_and_its_subprojects, @project.name), 'subprojects'] unless @project.nil? || @project.descendants.active.empty?
      searchableProjects << [@project.name, ''] unless @project.nil?
      return render json: {
        form: {
          question: @question,
          all_words: @all_words,
