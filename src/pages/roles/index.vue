  ...
  setup() {
    ...
    const getActionMenuItems = (
        ...
        return [
          {
            component: h(VListItem, {
              title: t("roleActionMenu.filterTo.title"),
      ...
      return [
        ...
        {
          component: h(VListItem, {
            title: t("roleActionMenu.delete.title"),
            appendIcon: "mdi-delete",
            density: "compact",
            onClick: async () => {
              if (!api || !swal || !toast) return;
              const { isConfirmed } = await swal.fire({
                title: t("roleActionMenu.delete.confirm"),
                icon: "warning",
              ...
              const { status } = await api.delete(
                `/roles/${customFields[0].id}?${qs.stringify(getCsrfObject())}`,
              );
              if (status < 200 || status >= 300) {
                toast.fire({
                  title: t("roleActionMenu.delete.error"),
                  icon: "error",
                });
              }
              onDone();
