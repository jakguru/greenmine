  ...
  setup() {
    ...
    const getActionMenuItems = (
        ...
        return [
          {
            component: h(VListItem, {
              title: t("groupActionMenu.filterTo.title"),
      ...
      return [
        ...
        {
          component: h(VListItem, {
            title: t("groupActionMenu.delete.title"),
            appendIcon: "mdi-delete",
            density: "compact",
            onClick: async () => {
              if (!api || !swal || !toast) return;
              const { isConfirmed } = await swal.fire({
                title: t("groupActionMenu.delete.confirm"),
                icon: "warning",
              ...
              const { status } = await api.delete(
                `/groups/${customFields[0].id}?${qs.stringify(getCsrfObject())}`,
              );
              if (status < 200 || status >= 300) {
                toast.fire({
                  title: t("groupActionMenu.delete.error"),
                  icon: "error",
                });
              }
              onDone();
