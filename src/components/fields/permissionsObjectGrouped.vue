      ...
      <v-sheet v-bind="vFieldFieldBindings">
        ...
        <v-table>
          <thead>
            <tr>
              <th>&nbsp;</th>
              <th v-for="field in fields" :key="`label-for-${field.value}`">
                {{ field.label }}
              </th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="tracker in trackers" :key="`tracker-${tracker.value}`">
              <td>
                {{ tracker.label }}
              </td>
              <td
                v-for="field in fields"
                :key="`tracker-${tracker.value}-switch-for-${field.value}`"
              >
                <v-switch
                  v-model="value[field.value]"
                  :value="tracker.value.toString()"
                  :disabled="disabledPermissions[field.value]"
                  :readonly="disabledPermissions[field.value]"
                  class="mx-auto"
                  multiple
                />
              </td>
...
export default defineComponent({
  name: "PermissionsObjectGroupedField",
  components: {},
  props: {
    appendIcon: {
    ...
    trackers: {
      type: Array as PropType<RoleValuesProp["trackers"]>,
      default: () => [],
    },
    model: {
  ...
  emits: {
    "update:modelValue": (val: Record<string, number[]>) =>
      "object" === typeof val && null !== val,
    "update:model-value": (val: Record<string, number[]>) =>
      "object" === typeof val && null !== val,
    update: (val: Record<string, number[]>) =>
      "object" === typeof val && null !== val,
  },
  setup(props, { emit }) {
    ...
    const value = ref<Record<string, number[]>>(props.modelValue);
    watch(
      () => props.modelValue,
      (is, was) => {
    ...
    const keys = computed(() => Object.keys(value.value));
      ...
      () =>
        [...keys.value]
          .sort((a, b) => {
            if (a === "view_issues") {
              return -1;
            } else if (a === "add_issues" && !["view_issues"].includes(b)) {
              return -1;
            } else if (
              a === "edit_issues" &&
              !["view_issues", "add_issues"].includes(b)
            ) {
              return -1;
            } else if (
              a === "add_issue_notes" &&
              !["view_issues", "add_issues", "edit_issues"].includes(b)
            ) {
              return -1;
            } else if (
              a === "delete_issues" &&
          ...
          .map((key) => permissions.value.find((p) => p.value === key))
          .filter((p) => p) as RoleValuesProp["permissions"],
    );
      ...
      Object.assign(
        ...
        ...[...keys.value].map((perm) => ({
          [perm]:
            ...
            model.value.permissions_all_trackers[perm] === "1",
        })),
      ),
    );
    return {
